...
**触发条件（2026-09-24 复核）**：M3 逐项评估结论——事件审计/webhooks/两段式/KVM/批量标签/i18n 均不需要后端（引擎是唯一事实源）；唯一压力点是**安装方案库的多操作者共享**。因此：v1 方案库走 localStorage + JSON 导入导出（UI 明示"保存在本浏览器"）；当"跨操作者/跨浏览器共享方案"成为真实诉求时引入 BFF。
