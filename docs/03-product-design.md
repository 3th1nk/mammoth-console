...
### 3.2 v2 预留：轻量 BFF（可选增强，触发条件明确，暂不做）

若未来需要多用户、模板服务端存储、root 密码安全托管、机器搜索代理，再引入 console BFF（Go 单二进制，代理引擎 + 自有用户表）。v1 的 API 层（生成的 TypeScript client）与页面解耦，保证届时迁移成本可控。

**触发条件（2026-09-24 复核）**：M3 逐项评估结论——事件审计/webhooks/两段式/KVM/批量标签/i18n 均不需要后端（引擎是唯一事实源）；唯一压力点是**安装方案库的多操作者共享**。因此：v1 方案库走 localStorage + JSON 导入导出（UI 明示"保存在本浏览器"）；当"跨操作者/跨浏览器共享方案"或"多用户"成为真实诉求时引入 BFF（2026-09-24：方案库已判定非刚需缓发，BFF 触发条件实际仅剩多用户等场景）。
...
| **M3 完善** | 事件审计、webhooks、批量标签（前端循环）、BIOS/擦盘两段式、KVM 降级、i18n(en)；**安装方案库缓发**（非刚需，见 §3.2——BFF 触发条件随之仅剩多用户等真实诉求） | 覆盖 openapi 全部面向管理员的端点 |
